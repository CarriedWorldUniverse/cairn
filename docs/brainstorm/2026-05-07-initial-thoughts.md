...
Three shapes:
- **A. Owned agents.** Each agent belongs to exactly one human user. New `agent` table, `user_id` FK. Commits authored by the agent; render as "Plumb (under jacinta)". Matches the existing team model.
...
Multi-agent-under-one-user is the load-bearing feature, and **the git commit history must show *which agent* wrote each commit** — not a flattened "everything by jacinta." `git log` on vanilla tooling, on a mirror, on GitHub if mirrored, on `git blame`, anywhere — the agent ID is visible in the artefact. No flattening to single-author at any point in the pipeline.

Concrete encoding (needs anvil's pass on the exact convention):

- **Author** = agent identity. Name carries the agent slug (`Plumb`); email is a stable agent-derived address (e.g. `plumb@agents.jacinta.cairn` or `jacinta+plumb@cairn`). Same convention everywhere agents commit.
...
- **Commit trailers** carry machine-readable metadata: `Agent-Id: plumb`, `Agent-Owner: jacinta`, optional `Agent-Capability: <scope>`. Trailers are stable across rebase / cherry-pick (unlike author overrides during merge), and parseable by external tooling.
...
Forgejo Actions runner placement. Heavy compute → dMon-Linux. Light → nexus-cw-ec2? Per-agent runner tokens? Forge owns this.
