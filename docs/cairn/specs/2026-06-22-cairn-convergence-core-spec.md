...
10. **Feature tiers by deployment — graceful degradation onto git.** *No origin (local):* full local convergence model. *Over a plain git origin (GitHub/GitLab):* local model + standard clone/push/origin-sync, but **server-enforced features are unavailable** — a dumb git remote can't hold a separate private store, emit a redacted projection, or gate access, so **privacy/embargo/`Disclose` and herald-scoped private content require a cairn server**. *With a cairn server:* the full set (privacy enforcement + the multi-agent hub). cairn degrades gracefully onto git and lights up fully on a cairn server.
11. **First build = Phase 1, the convergence core** (incl. lines/branches + tags + lineage), proven with a concurrency test harness — no porter, no ledger needed to validate.
...
- **Private (embargoed) commit** → freezes only the **public** projection, **not distribution.** The *public* view of that line is held at the embargoed commit's parent — nothing at or after it is shared **publicly** (including later commits that descend from it) until `Disclose`, which then advances the public tip to the next boundary (or the line tip). **But the embargoed commits remain fully live to the owner and authorized recipients** — they clone, build, ship, and deploy them *now*, through the cairn server / authorized channels. **This is the entire point (NEX-25 patch-gap):** get a security patch into the hands of the people who need it *before* the fix is visible in the public repo, so an attacker can't patch-diff it into an n-day. Embargo = "patched in private, revealed in public later," never "undistributed." (The public boundary stays consistent and buildable; you just can't publish a half-disclosed tree *publicly*.)

So a public clone contains: public content in full, the **shape** of private parts (paths/structure), and **none of their content** — there is nothing to spelunk. The real private blobs/trees **never enter the public packfile**; authorized clients obtain them from the private store via cairn (herald-scoped), never from a public clone. (Per-flag, a path may escalate from shape-only to **fully omitted** — not even the path appears — for truly secret items; default is shape-only per "know the shape, not the contents." Encryption-in-place (e.g. casket ciphertext in the objects) is **explicitly rejected, not a future variant**: it doesn't remove the secret, it relocates it to a key that then needs its own custody/sharing/loss story — solving nothing and adding a layer. The rule is flat: **private content is withheld from any git push, full stop — encryption is never a substitute for absence.** Over a plain git origin with no cairn server, this *is* the entire privacy behaviour — private paths are tracked locally and simply never shipped (cairn enforces the boundary the way a smarter `.gitignore` would); the redacted projection, gating, and embargo/`Disclose` are cairn-**server** features.)

This is the load-bearing **delayed-public-projection** mechanism (NEX-25), now at folder/file/commit granularity. **Phase 1 reserves the object-graph-rewrite hook** — export must be able to emit a *redacted graph*, not merely select refs; the redactor + flag store land in Phase 3. **Hard invariant: no private byte — plaintext *or* ciphertext — ever exists in a public projection's objects.**
...
**Privacy interaction.** Two release paths from the same derived version:
- **Authorized / embargoed release (now):** the owner can `cairn release` an embargoed patch to **authorized recipients** immediately — via the cairn server / a private feed / authorized fetch — so customers deploy the fix *before* it's public (the patch-gap point, §6). The derived version is real and stamped; only its *public* exposure is held.
- **Public release (on `Disclose`):** the **public** registry/version line derives from the **public projection**, so it stays frozen at the embargo boundary and only advances when the embargo lifts — public package versions never get ahead of what's disclosed.

So an embargoed security version exists and ships to those who need it immediately, and appears on the public feed only at disclosure — same version number, two timelines.
