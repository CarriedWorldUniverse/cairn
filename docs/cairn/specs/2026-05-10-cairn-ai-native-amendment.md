...
- **`cairn_summarizer_config`** — per-org row:
  - `owner_id` (FK to user/org), `enabled` (bool), `provider` (bridle ProviderID string: claude-code | openai-api | claude-api | bedrock | ollama-local), `endpoint_url` (text, optional), `model_id` (text), `credentials` (encrypted), `levels_enabled` (bitfield: PR | commit | file), `created_at`, `updated_at`
