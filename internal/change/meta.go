...
func (e *Engine) ExportMeta() (string, error) {
	doc, err := e.buildMetaDoc()
	if err != nil {
		return "", err
	}
	return e.writeMetaDoc(doc)
}

// buildMetaDoc reads the catalogue into a metaDoc (deterministic ordering).
func (e *Engine) buildMetaDoc() (metaDoc, error) {
	...
	if err != nil {
		return metaDoc{}, fmt.Errorf("change.ExportMeta: lines: %w", err)
	}
	for rows.Next() {
		var l metaLine
	...
	if err != nil {
		return metaDoc{}, fmt.Errorf("change.ExportMeta: changes: %w", err)
	}
	for crows.Next() {
		var c metaChange
		var sealed, hc int
	...
	if err != nil {
		return metaDoc{}, fmt.Errorf("change.ExportMeta: conflicts: %w", err)
	}
	for frows.Next() {
		var c metaConf
...
// writeMetaDoc serializes a metaDoc to its deterministic git commit and returns
// the sha. The fixed epoch + fixed signature + no Change-Id trailer make the hash
// a pure function of the doc, so an unchanged catalogue re-exports identically.
func (e *Engine) writeMetaDoc(doc metaDoc) (string, error) {
