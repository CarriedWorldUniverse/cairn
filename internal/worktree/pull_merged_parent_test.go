...
// The conflicted variant (#195): a sealed local change to a file main also
// changed makes pull record a merge [local, upstream] with a conflict. While
// it is open, status must list the conflict and the operator's own work — not
// the files the pull merely brought in — and editing the folder must not
// drop the upstream side: that re-marked the resolved conflict on commit and
// left a push that could not fast-forward.
func TestConflictedPullKeepsUpstreamThroughEditResolveAndCommit(t *testing.T) {
	if _, err := exec.LookPath("git"); err != nil {
		t.Skip("git CLI not available")
	}
	origin := looseTempDir(t)
	gitIn(t, origin, "init", "-q", "-b", "main")
	writeIn(t, origin, "s.txt", "one\ntwo\nthree\n")
	writeIn(t, origin, "other.txt", "x\n")
	gitIn(t, origin, "add", ".")
	gitIn(t, origin, "commit", "-qm", "seed")
	gitIn(t, origin, "checkout", "-qb", "pr")
	writeIn(t, origin, "p.txt", "p\n")
	gitIn(t, origin, "add", ".")
	gitIn(t, origin, "commit", "-qm", "pr1")
	gitIn(t, origin, "checkout", "-q", "main")
	...
	dir := filepath.Join(r.Root(), r.st.Expressed["pr"].Path)
	writeIn(t, dir, "s.txt", "one\ntwo\nthree-local\n")
	if _, err := r.Commit("pr", "self change"); err != nil {
		t.Fatalf("Commit: %v", err)
	}

	writeIn(t, origin, "s.txt", "one\ntwo\nthree-main\n")
	writeIn(t, origin, "other.txt", "y\n")
	gitIn(t, origin, "commit", "-qam", "main change")
	gitIn(t, origin, "checkout", "-q", "pr")
	gitIn(t, origin, "merge", "-q", "--no-edit", "main")
	gitIn(t, origin, "checkout", "-q", "main")
	...
	if !reflect.DeepEqual(st.Conflicts, []string{"s.txt"}) {
		t.Fatalf("conflicts = %v, want [s.txt]", st.Conflicts)
	}
	if !reflect.DeepEqual(st.Modified, []string{"s.txt"}) || len(st.Added)+len(st.Deleted) != 0 {
		t.Fatalf("status A=%v M=%v D=%v, want only M [s.txt] — other.txt came from upstream, it is not a local change",
			st.Added, st.Modified, st.Deleted)
	}

	// Edit the conflict away and let a status snapshot it BEFORE resolving —
	// the step that used to drop the upstream parent.
	writeIn(t, dir, "s.txt", "one\ntwo\nthree-both\n")
	if _, err := r.Status("pr"); err != nil {
		t.Fatalf("Status: %v", err)
	}
	if err := r.Resolve("pr", "s.txt", false); err != nil {
		t.Fatalf("Resolve: %v", err)
	}
	res, err := r.Commit("pr", "resolve")
	if err != nil {
		t.Fatalf("Commit: %v", err)
	}
	if len(res.Conflicts) != 0 {
		t.Fatalf("commit re-marked %d conflict(s) the operator had resolved: %v", len(res.Conflicts), res.Conflicts)
	}
	if got, _ := os.ReadFile(filepath.Join(dir, "s.txt")); string(got) != "one\ntwo\nthree-both\n" {
		t.Fatalf("s.txt after commit = %q, want the resolution", got)
	}
	// The upstream tip must still be in the line's history, so the push
	// fast-forwards the branch instead of being rejected.
	if err := r.PushBranch("origin", "pr", false); err != nil {
		t.Fatalf("push after resolving a pulled conflict: %v", err)
	}
}
